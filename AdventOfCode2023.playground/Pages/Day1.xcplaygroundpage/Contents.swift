...
let calibrations = input.components(separatedBy: "\n")

/// PART 1

calibrations.reduce(0) { partialResult, calibration in
    ...
    if let firstNum = directions.first, let secondNum = directions.last {
        newNum = Int("\(firstNum)\(secondNum)")!
    }
    
    return partialResult + newNum
}
...
let testCalibrations = input.components(separatedBy: "\n")
